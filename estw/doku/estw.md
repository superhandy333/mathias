...
|-|-|-|
|PR|Rangierstraße zulässig (Gleis darf Element einer Rangierstrasse sein)|Gleis|
...
|PZSP|Zielsperre (Signal kann nicht gleichzeitig Flankenschutzelement und Fahrstraßenziel sein)|Signal|
...
- **Speichern** prüft und übernimmt die Arbeitsdaten in die aktive Datenquelle und schließt bei Erfolg beide Editorfenster. **Verwerfen** beendet den Modus ohne Übernahme der Datenänderungen. Bei ungespeicherten Änderungen verhindert die Schließen-Schaltfläche der Lupe ein unbeabsichtigtes Verwerfen und verweist auf diese beiden Aktionen. Ein ausgeblendetes Werkzeugfenster lässt sich über **Ansicht → Werkzeugfenster anzeigen** wieder öffnen.

**Arbeitsdaten und Validierung:** [FahrstrassenEditor](../fahrstrasseneditor.h) hält eine Kopie der ursprünglichen und bearbeiteten Fahrstrassen samt Zuordnungen. Neue Datensätze erhalten zunächst negative, nur im Editor verwendete IDs. Unvollständige Fahrstrassen dürfen während der Bearbeitung bestehen; beim Speichern gelten die vorhandenen Regeln aus [rules_isvalid_estwdata.md](rules_isvalid_estwdata.md), insbesondere genau ein gültiges Start-, Ziel- und Auflöseelement. Die zusätzlichen Rollen 5 bis 8 sind nur für Zugstrassen zulässig. Bei Validierungs- oder Speicherfehlern bleibt der Editor mit seinen Arbeitsdaten geöffnet.
